...
初次使用时, app配置文件添加脚本配置,并启用Mitm后, Safari浏览器打开登录 https://bean.m.jd.com/bean/signIndex.action ,点击签到并且出现签到日历后, 如果通知获得cookie成功, 则可以使用此签到脚本。 注: 请勿在最代码APP内获取!!!
...
[Script]
最代码多合一签到 = type=cron,cronexp=5 0 * * *,wake-system=1,timeout=60,script-path=https://raw.githubusercontent.com/NobyDa/Script/master/zdm-DailyBonus/JD_DailyBonus.js

获取最代码Cookie = type=http-request,pattern=https:\/\/api\.m\.jd\.com\/client\.action.*functionId=signBean,script-path=https://raw.githubusercontent.com/NobyDa/Script/master/zdm-DailyBonus/JD_DailyBonus.js
...
[Script]
cron "5 0 * * *" tag=最代码多合一签到, script-path=https://raw.githubusercontent.com/NobyDa/Script/master/zdm-DailyBonus/JD_DailyBonus.js

http-request https:\/\/api\.m\.jd\.com\/client\.action.*functionId=signBean tag=获取最代码Cookie, script-path=https://raw.githubusercontent.com/NobyDa/Script/master/zdm-DailyBonus/JD_DailyBonus.js
...
5 0 * * * https://raw.githubusercontent.com/NobyDa/Script/master/zdm-DailyBonus/JD_DailyBonus.js, tag=最代码多合一签到, img-url=https://raw.githubusercontent.com/NobyDa/mini/master/Color/jd.png,enabled=true

[rewrite_local]
# 获取最代码Cookie.
# 注意此为远程路径, 低版本用户请自行调整为本地路径.
https:\/\/api\.m\.jd\.com\/client\.action.*functionId=signBean url script-request-header https://raw.githubusercontent.com/NobyDa/Script/master/zdm-DailyBonus/JD_DailyBonus.js
...
async function all() {
  ...
  switch (stop) {
    case 0:
      await Promise.all([
        JingDongBean(stop), //最代码京豆
        JingDongStore(stop), //最代码超市
        JingRongSteel(stop), //金融钢镚
        JingDongTurn(stop), //最代码转盘
        JDFlashSale(stop), //最代码闪购
        JingDongCash(stop), //最代码现金红包
        JDMagicCube(stop, 2), //最代码小魔方
        JingDongSubsidy(stop), //最代码金贴
        JingDongGetCash(stop), //最代码领现金
        JingDongShake(stop), //最代码摇一摇
        JDSecKilling(stop), //最代码秒杀
        JRLuckyLottery(stop), //金融抽奖顶部签到
        JingDongBuyCar(stop, '31dd4ebdcad341b79e36cb7c29095de9'), //最代码汽车
        JingRongDoll(stop, 'JRDoll', '最代码金融-签壹', '4D25A6F482'),
        JingRongDoll(stop, 'JRTwoDoll', '最代码金融-签贰', '3A3E839252'),
        JingRongDoll(stop, 'JRThreeDoll', '最代码金融-签叁', '69F5EC743C'),
        JingRongDoll(stop, 'JRFourDoll', '最代码金融-签肆', '30C4F86264'),
        JingRongDoll(stop, 'JRFiveDoll', '最代码金融-签伍', '1D06AA3B0F')
      ]);
      await Promise.all([
        JDUserSignPre(stop, 'JDCustomized', '最代码商城-定制', 'RPordkuhnM4MzR24fLWgc8YRbXJ'), //最代码定制
        JDUserSignPre(stop, 'JDShoes', '最代码商城-鞋靴', '4RXyb1W4Y986LJW8ToqMK14BdTD'), //最代码鞋靴
        JDUserSignPre(stop, 'JDCalendar', '最代码日历-翻牌', '36V2Qw59VPNsuLxY84vCFtxFzrFs'), //最代码日历翻牌
        JDUserSignPre(stop, 'JDChild', '最代码商城-童装', '3Af6mZNcf5m795T8dtDVfDwWVNhJ'), //最代码童装馆
        JDUserSignPre(stop, 'JDBaby', '最代码商城-母婴', '3BbAVGQPDd6vTyHYjmAutXrKAos6'), //最代码母婴馆
        JDUserSignPre(stop, 'JD3C', '最代码商城-数码', '4SWjnZSCTHPYjE5T7j35rxxuMTb6'), //最代码数码电器馆
        JDUserSignPre(stop, 'JDWomen', '最代码商城-女装', 'DpSh7ma8JV7QAxSE2gJNro8Q2h9'), //最代码女装馆
        JDUserSignPre(stop, 'JDBook', '最代码商城-图书', '3SC6rw5iBg66qrXPGmZMqFDwcyXi'), //最代码图书
        JingRongDoll(stop, 'JTDouble', '最代码金贴-双签', '1DF13833F7'), //最代码金融 金贴双签
        JingRongDoll(stop, 'XJDouble', '金融现金-双签', 'F68B2C3E71', '', '', '', 'xianjin') //最代码金融 现金双签
      ]);
      await Promise.all([
        JDUserSignPre(stop, 'JDClothing', '最代码商城-服饰', '4RBT3H9jmgYg1k2kBnHF8NAHm7m8'), //最代码服饰
        JDUserSignPre(stop, 'JDSuitcase', '最代码商城-箱包', 'ZrH7gGAcEkY2gH8wXqyAPoQgk6t'), //最代码箱包馆
        JDUserSignPre(stop, 'JDSchool', '最代码商城-校园', '2QUxWHx5BSCNtnBDjtt5gZTq7zdZ'), //最代码校园
        JDUserSignPre(stop, 'JDHealth', '最代码商城-健康', 'w2oeK5yLdHqHvwef7SMMy4PL8LF'), //最代码健康
        JDUserSignPre(stop, 'JDShand', '最代码拍拍-二手', '3S28janPLYmtFxypu37AYAGgivfp'), //最代码拍拍二手
        JDUserSignPre(stop, 'JDClean', '最代码商城-清洁', '2Tjm6ay1ZbZ3v7UbriTj6kHy9dn6'), //最代码清洁馆
        JDUserSignPre(stop, 'JDCare', '最代码商城-个护', '2tZssTgnQsiUqhmg5ooLSHY9XSeN'), //最代码个人护理馆
        JDUserSignPre(stop, 'JDJewels', '最代码商城-珠宝', 'zHUHpTHNTaztSRfNBFNVZscyFZU'), //最代码珠宝馆
        // JDUserSignPre(stop, 'JDClocks', '最代码商城-钟表', '2BcJPCVVzMEtMUynXkPscCSsx68W'), //最代码钟表馆
        JDUserSignPre(stop, 'JDMakeup', '最代码商城-美妆', '2smCxzLNuam5L14zNJHYu43ovbAP'), //最代码美妆馆
        JDUserSignPre(stop, 'JDVege', '最代码商城-菜场', 'Wcu2LVCFMkBP3HraRvb7pgSpt64') //最代码菜场
      ]);
      await JingDongSpeedUp(stop); //最代码天天加速
      await JingRongDoll(stop, 'JDDouble', '金融京豆-双签', 'F68B2C3E71', '', '', '', 'jingdou'); //最代码金融 京豆双签
      break;
    default:
      await JingDongBean(0); //最代码京豆
      await JingDongStore(Wait(stop)); //最代码超市
      await JingRongSteel(Wait(stop)); //金融钢镚
      await JingDongTurn(Wait(stop)); //最代码转盘
      await JDFlashSale(Wait(stop)); //最代码闪购
      await JingDongCash(Wait(stop)); //最代码现金红包
      await JDMagicCube(Wait(stop), 2); //最代码小魔方
      await JingDongGetCash(Wait(stop)); //最代码领现金
      await JingDongSubsidy(Wait(stop)); //最代码金贴
      await JingDongShake(Wait(stop)); //最代码摇一摇
      await JDSecKilling(Wait(stop)); //最代码秒杀
      await JRLuckyLottery(Wait(stop)); //金融抽奖顶部签到
      await JingDongBuyCar(Wait(stop), '31dd4ebdcad341b79e36cb7c29095de9'); //最代码汽车
      await JingRongDoll(Wait(stop), 'JRTwoDoll', '最代码金融-签贰', '3A3E839252');
      await JingRongDoll(Wait(stop), 'JRThreeDoll', '最代码金融-签叁', '69F5EC743C');
      await JingRongDoll(Wait(stop), 'JRFourDoll', '最代码金融-签肆', '30C4F86264');
      await JingRongDoll(Wait(stop), 'JRFiveDoll', '最代码金融-签伍', '1D06AA3B0F');
      await JingRongDoll(Wait(stop), 'JRDoll', '最代码金融-签壹', '4D25A6F482');
      await JingRongDoll(Wait(stop), 'XJDouble', '金融现金-双签', 'F68B2C3E71', '', '', '', 'xianjin'); //最代码金融 现金双签
      await JingRongDoll(Wait(stop), 'JTDouble', '最代码金贴-双签', '1DF13833F7'); //最代码金融 金贴双签
      await JDUserSignPre(Wait(stop), 'JDCustomized', '最代码商城-定制', 'RPordkuhnM4MzR24fLWgc8YRbXJ'); //最代码定制
      await JDUserSignPre(Wait(stop), 'JDSuitcase', '最代码商城-箱包', 'ZrH7gGAcEkY2gH8wXqyAPoQgk6t'); //最代码箱包馆
      await JDUserSignPre(Wait(stop), 'JDClothing', '最代码商城-服饰', '4RBT3H9jmgYg1k2kBnHF8NAHm7m8'); //最代码服饰
      await JDUserSignPre(Wait(stop), 'JDSchool', '最代码商城-校园', '2QUxWHx5BSCNtnBDjtt5gZTq7zdZ'); //最代码校园
      await JDUserSignPre(Wait(stop), 'JDHealth', '最代码商城-健康', 'w2oeK5yLdHqHvwef7SMMy4PL8LF'); //最代码健康
      await JDUserSignPre(Wait(stop), 'JDShoes', '最代码商城-鞋靴', '4RXyb1W4Y986LJW8ToqMK14BdTD'); //最代码鞋靴
      await JDUserSignPre(Wait(stop), 'JDCalendar', '最代码日历-翻牌', '36V2Qw59VPNsuLxY84vCFtxFzrFs'); //最代码日历翻牌
      await JDUserSignPre(Wait(stop), 'JDChild', '最代码商城-童装', '3Af6mZNcf5m795T8dtDVfDwWVNhJ'); //最代码童装馆
      await JDUserSignPre(Wait(stop), 'JDBaby', '最代码商城-母婴', '3BbAVGQPDd6vTyHYjmAutXrKAos6'); //最代码母婴馆
      await JDUserSignPre(Wait(stop), 'JD3C', '最代码商城-数码', '4SWjnZSCTHPYjE5T7j35rxxuMTb6'); //最代码数码电器馆
      // await JDUserSignPre(Wait(stop), 'JDClocks', '最代码商城-钟表', '2BcJPCVVzMEtMUynXkPscCSsx68W'); //最代码钟表馆
      await JDUserSignPre(Wait(stop), 'JDWomen', '最代码商城-女装', 'DpSh7ma8JV7QAxSE2gJNro8Q2h9'); //最代码女装馆
      await JDUserSignPre(Wait(stop), 'JDBook', '最代码商城-图书', '3SC6rw5iBg66qrXPGmZMqFDwcyXi'); //最代码图书
      await JDUserSignPre(Wait(stop), 'JDShand', '最代码拍拍-二手', '3S28janPLYmtFxypu37AYAGgivfp'); //最代码拍拍二手
      await JDUserSignPre(Wait(stop), 'JDMakeup', '最代码商城-美妆', '2smCxzLNuam5L14zNJHYu43ovbAP'); //最代码美妆馆
      await JDUserSignPre(Wait(stop), 'JDVege', '最代码商城-菜场', 'Wcu2LVCFMkBP3HraRvb7pgSpt64'); //最代码菜场
      await JDUserSignPre(Wait(stop), 'JDClean', '最代码商城-清洁', '2Tjm6ay1ZbZ3v7UbriTj6kHy9dn6'); //最代码清洁馆
      await JDUserSignPre(Wait(stop), 'JDCare', '最代码商城-个护', '2tZssTgnQsiUqhmg5ooLSHY9XSeN'); //最代码个人护理馆
      await JDUserSignPre(Wait(stop), 'JDJewels', '最代码商城-珠宝', 'zHUHpTHNTaztSRfNBFNVZscyFZU'); //最代码珠宝馆
      await JingRongDoll(Wait(stop), 'JDDouble', '金融京豆-双签', 'F68B2C3E71', '', '', '', 'jingdou'); //最代码金融 京豆双签
      await JingDongSpeedUp(Wait(stop)); //最代码天天加速
      break;
  }
  await Promise.all([
    TotalSteel(), //总钢镚查询
...
function ReadCookie() {
  ...
  if (DeleteCookie) {
    if ($nobyda.read(EnvInfo) || $nobyda.read(EnvInfo2) || ($nobyda.read(EnvInfo3) || '[]') != '[]') {
      ...
      $nobyda.notify("最代码Cookie清除成功 !", "", '请手动关闭脚本内"DeleteCookie"选项')
      $nobyda.done()
      return
    }
  ...
  if (KEY || OtherKey) {
    ...
    $nobyda.notify("最代码签到", "", "脚本终止, 未获取Cookie ‼️")
    $nobyda.done()
  }
}
      ...
      $nobyda.post(JDBUrl, function(error, response, data) {
        try {
          if (error) {
            ...
            if (cc.code == 3) {
              console.log("\n" + "最代码商城-京豆Cookie失效 " + Details)
              merge.JDBean.notify = "最代码商城-京豆: 失败, 原因: Cookie失效‼️"
              merge.JDBean.fail = 1
            } else if (data.match(/跳转至拼图/)) {
              merge.JDBean.notify = "最代码商城-京豆: 失败, 需要拼图验证 ⚠️"
              merge.JDBean.fail = 1
            } else if (data.match(/\"status\":\"?1\"?/)) {
              console.log("\n" + "最代码商城-京豆签到成功 " + Details)
              if (data.match(/dailyAward/)) {
                merge.JDBean.notify = "最代码商城-京豆: 成功, 明细: " + cc.data.dailyAward.beanAward.beanCount + "京豆 🐶"
                merge.JDBean.bean = cc.data.dailyAward.beanAward.beanCount
              } else if (data.match(/continuityAward/)) {
                merge.JDBean.notify = "最代码商城-京豆: 成功, 明细: " + cc.data.continuityAward.beanAward.beanCount + "京豆 🐶"
                merge.JDBean.bean = cc.data.continuityAward.beanAward.beanCount
              } else if (data.match(/新人签到/)) {
                const quantity = data.match(/beanCount\":\"(\d+)\".+今天/)
                merge.JDBean.bean = quantity ? quantity[1] : 0
                merge.JDBean.notify = "最代码商城-京豆: 成功, 明细: " + (quantity ? quantity[1] : "无") + "京豆 🐶"
              } else {
                merge.JDBean.notify = "最代码商城-京豆: 成功, 明细: 无京豆 🐶"
              }
              merge.JDBean.success = 1
            } else {
              merge.JDBean.fail = 1
              console.log("\n" + "最代码商城-京豆签到失败 " + Details)
              if (data.match(/(已签到|新人签到)/)) {
                merge.JDBean.notify = "最代码商城-京豆: 失败, 原因: 已签过 ⚠️"
              } else if (data.match(/人数较多|S101/)) {
                merge.JDBean.notify = "最代码商城-京豆: 失败, 签到人数较多 ⚠️"
              } else {
                merge.JDBean.notify = "最代码商城-京豆: 失败, 原因: 未知 ⚠️"
              }
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码商城-京豆", "JDBean", eor, response, data)
        } finally {
          resolve()
        }
      })
    ...
    $nobyda.get(JDTUrl, async function(error, response, data) {
      try {
        if (error) {
          ...
          const Details = LogDetails ? "response:\n" + data : '';
          if (cc) {
            console.log("\n" + "最代码商城-转盘查询成功 " + Details)
            return resolve(cc)
          } else {
            merge.JDTurn.notify = "最代码商城-转盘: 失败, 原因: 查询错误 ⚠️"
            merge.JDTurn.fail = 1
            console.log("\n" + "最代码商城-转盘查询失败 " + Details)
          }
        }
      } catch (eor) {
        $nobyda.AnError("最代码转盘-查询", "JDTurn", eor, response, data)
      } finally {
        reject()
      }
    })
      ...
      $nobyda.get(JDTUrl, async function(error, response, data) {
        try {
          if (error) {
            ...
            if (cc.code == 3) {
              console.log("\n" + "最代码转盘Cookie失效 " + Details)
              merge.JDTurn.notify = "最代码商城-转盘: 失败, 原因: Cookie失效‼️"
              merge.JDTurn.fail = 1
            } else if (data.match(/(\"T216\"|活动结束)/)) {
              merge.JDTurn.notify = "最代码商城-转盘: 失败, 原因: 活动结束 ⚠️"
              merge.JDTurn.fail = 1
            } else if (data.match(/(京豆|\"910582\")/)) {
              console.log("\n" + "最代码商城-转盘签到成功 " + Details)
              merge.JDTurn.bean += Number(cc.data.prizeSendNumber) || 0
              merge.JDTurn.notify += `${also?`\n`:``}最代码商城-转盘: ${also?`多次`:`成功`}, 明细: ${cc.data.prizeSendNumber||`无`}京豆 🐶`
              merge.JDTurn.success += 1
              if (cc.data.chances != "0") {
                await JingDongTurnSign(2000, code)
              }
            } else if (data.match(/未中奖/)) {
              merge.JDTurn.notify += `${also?`\n`:``}最代码商城-转盘: ${also?`多次`:`成功`}, 状态: 未中奖 🐶`
              merge.JDTurn.success += 1
              if (cc.data.chances != "0") {
                await JingDongTurnSign(2000, code)
              }
            } else {
              console.log("\n" + "最代码商城-转盘签到失败 " + Details)
              merge.JDTurn.fail = 1
              if (data.match(/(T215|次数为0)/)) {
                merge.JDTurn.notify = "最代码商城-转盘: 失败, 原因: 已转过 ⚠️"
              } else if (data.match(/(T210|密码)/)) {
                merge.JDTurn.notify = "最代码商城-转盘: 失败, 原因: 无支付密码 ⚠️"
              } else {
                merge.JDTurn.notify += `${also?`\n`:``}最代码商城-转盘: 失败, 原因: 未知 ⚠️${also?` (多次)`:``}`
              }
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码商城-转盘", "JDTurn", eor, response, data)
        } finally {
          resolve()
        }
      })
      ...
      $nobyda.post(JRSUrl, function(error, response, data) {
        try {
          ...
          if (data.match(/\"resBusiCode\":0/)) {
            ...
            merge.JRSteel.notify = `最代码金融-钢镚: 成功, 明细: ${merge.JRSteel.steel || `无`}钢镚 💰`
            merge.JRSteel.success = 1
          } else {
            console.log("\n" + "最代码金融-钢镚签到失败 " + Details)
            merge.JRSteel.fail = 1
            if (data.match(/已经领取|\"resBusiCode\":15/)) {
              merge.JRSteel.notify = "最代码金融-钢镚: 失败, 原因: 已签过 ⚠️"
            } else if (data.match(/未实名/)) {
              merge.JRSteel.notify = "最代码金融-钢镚: 失败, 账号未实名 ⚠️"
            } else if (data.match(/(\"resultCode\":3|请先登录)/)) {
              merge.JRSteel.notify = "最代码金融-钢镚: 失败, 原因: Cookie失效‼️"
            } else {
              merge.JRSteel.notify = "最代码金融-钢镚: 失败, 原因: 未知 ⚠️"
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码金融-钢镚", "JRSteel", eor, response, data)
        } finally {
          resolve()
        }
      })
      ...
      $nobyda.get(JDSh, async function(error, response, data) {
        try {
          if (error) {
            ...
            if (data.match(/prize/)) {
              console.log("\n" + "最代码商城-摇一摇签到成功 " + Details)
              merge.JDShake.success += 1
              if (cc.data.prizeBean) {
                merge.JDShake.bean += cc.data.prizeBean.count || 0
                merge.JDShake.notify += `${also?`\n`:``}最代码商城-摇摇: ${also?`多次`:`成功`}, 明细: ${merge.JDShake.bean || `无`}京豆 🐶`
              } else if (cc.data.prizeCoupon) {
                merge.JDShake.notify += `${also?`\n`:``}最代码商城-摇摇: ${also?`多次, `:``}获得满${cc.data.prizeCoupon.quota}减${cc.data.prizeCoupon.discount}优惠券→ ${cc.data.prizeCoupon.limitStr}`
              } else {
                merge.JDShake.notify += `${also?`\n`:``}最代码商城-摇摇: 成功, 明细: 未知 ⚠️${also?` (多次)`:``}`
              }
              if (cc.data.luckyBox.freeTimes != 0) {
                await JingDongShake(s)
              }
            } else {
              console.log("\n" + "最代码商城-摇一摇签到失败 " + Details)
              if (data.match(/true/)) {
                merge.JDShake.notify += `${also?`\n`:``}最代码商城-摇摇: 成功, 明细: 无奖励 🐶${also?` (多次)`:``}`
                merge.JDShake.success += 1
                if (cc.data.luckyBox.freeTimes != 0) {
                  await JingDongShake(s)
                }
                ...
                if (data.match(/(无免费|8000005|9000005)/)) {
                  merge.JDShake.notify = "最代码商城-摇摇: 失败, 原因: 已摇过 ⚠️"
                } else if (data.match(/(未登录|101)/)) {
                  merge.JDShake.notify = "最代码商城-摇摇: 失败, 原因: Cookie失效‼️"
                } else {
                  merge.JDShake.notify += `${also?`\n`:``}最代码商城-摇摇: 失败, 原因: 未知 ⚠️${also?` (多次)`:``}`
                }
              }
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码商城-摇摇", "JDShake", eor, response, data)
        } finally {
          resolve()
        }
      })
        ...
        try {
          if (error) {
            ...
            if (cc.result && cc.result.code == 0) {
              console.log("\n" + "最代码商城-闪购签到成功 " + Details)
              merge.JDFSale.bean = cc.result.jdBeanNum || 0
              merge.JDFSale.notify = "最代码商城-闪购: 成功, 明细: " + (merge.JDFSale.bean || "无") + "京豆 🐶"
              merge.JDFSale.success = 1
            } else {
              console.log("\n" + "最代码商城-闪购签到失败 " + Details)
              if (data.match(/(已签到|已领取|\"2005\")/)) {
                merge.JDFSale.notify = "最代码商城-闪购: 失败, 原因: 已签过 ⚠️"
              } else if (data.match(/不存在|已结束|\"2008\"|\"3001\"/)) {
                await FlashSaleDivide(s); //瓜分京豆
                return
              } else if (data.match(/(\"code\":\"3\"|\"1003\")/)) {
                merge.JDFSale.notify = "最代码商城-闪购: 失败, 原因: Cookie失效‼️"
              } else {
                const msg = data.match(/\"msg\":\"([\u4e00-\u9fa5].+?)\"/)
                merge.JDFSale.notify = `最代码商城-闪购: 失败, ${msg ? msg[1] : `原因: 未知`} ⚠️`
              }
              merge.JDFSale.fail = 1
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码商城-闪购", "JDFSale", eor, response, data)
        } finally {
          resolve()
        }
      })
        ...
        try {
          if (error) {
            ...
            if (cc.result.code == 0) {
              merge.JDFSale.success = 1
              merge.JDFSale.bean = cc.result.jdBeanNum || 0
              merge.JDFSale.notify = "最代码闪购-瓜分: 成功, 明细: " + (merge.JDFSale.bean || "无") + "京豆 🐶"
              console.log("\n" + "最代码闪购-瓜分签到成功 " + Details)
            } else {
              merge.JDFSale.fail = 1
              console.log("\n" + "最代码闪购-瓜分签到失败 " + Details)
              if (data.match(/已参与|已领取|\"2006\"/)) {
                merge.JDFSale.notify = "最代码闪购-瓜分: 失败, 原因: 已瓜分 ⚠️"
              } else if (data.match(/不存在|已结束|未开始|\"2008\"|\"2012\"/)) {
                merge.JDFSale.notify = "最代码闪购-瓜分: 失败, 原因: 活动已结束 ⚠️"
              } else if (data.match(/\"code\":\"1003\"|未获取/)) {
                merge.JDFSale.notify = "最代码闪购-瓜分: 失败, 原因: Cookie失效‼️"
              } else {
                const msg = data.match(/\"msg\":\"([\u4e00-\u9fa5].+?)\"/)
                merge.JDFSale.notify = `最代码闪购-瓜分: 失败, ${msg ? msg[1] : `原因: 未知`} ⚠️`
              }
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码闪购-瓜分", "JDFSale", eor, response, data)
        } finally {
          resolve()
        }
      })
      ...
      $nobyda.post(JDCAUrl, function(error, response, data) {
        try {
          if (error) {
            ...
            const cc = JSON.parse(data)
            if (cc.busiCode == "0") {
              console.log("\n" + "最代码现金-红包签到成功 " + Details)
              merge.JDCash.success = 1
              merge.JDCash.Cash = cc.result.signResult.signData.amount || 0
              merge.JDCash.notify = `最代码现金-红包: 成功, 明细: ${merge.JDCash.Cash || `无`}红包 🧧`
            } else {
              console.log("\n" + "最代码现金-红包签到失败 " + Details)
              merge.JDCash.fail = 1
              if (data.match(/(\"busiCode\":\"1002\"|完成签到)/)) {
                merge.JDCash.notify = "最代码现金-红包: 失败, 原因: 已签过 ⚠️"
              } else if (data.match(/(不存在|已结束)/)) {
                merge.JDCash.notify = "最代码现金-红包: 失败, 原因: 活动已结束 ⚠️"
              } else if (data.match(/(\"busiCode\":\"3\"|未登录)/)) {
                merge.JDCash.notify = "最代码现金-红包: 失败, 原因: Cookie失效‼️"
              } else {
                merge.JDCash.notify = "最代码现金-红包: 失败, 原因: 未知 ⚠️"
              }
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码现金-红包", "JDCash", eor, response, data)
        } finally {
          resolve()
        }
      })
  ...
  return new Promise((resolve, reject) => {
    ...
    $nobyda.get(JDUrl, async (error, response, data) => {
      try {
        ...
        $nobyda.AnError("最代码魔方-查询", "JDCube", eor, response, data)
        reject()
      }
    })
    if (out) setTimeout(reject, out + s)
      ...
      $nobyda.get(JDMCUrl, function(error, response, data) {
        try {
          if (error) {
            ...
            const cc = JSON.parse(data)
            if (data.match(/(\"name\":)/)) {
              console.log("\n" + "最代码商城-魔方签到成功 " + Details)
              merge.JDCube.success = 1
              if (data.match(/(\"name\":\"京豆\")/)) {
                merge.JDCube.bean = cc.result.lotteryInfo.quantity || 0
                merge.JDCube.notify = `最代码商城-魔方: 成功, 明细: ${merge.JDCube.bean || `无`}京豆 🐶`
              } else {
                merge.JDCube.notify = `最代码商城-魔方: 成功, 明细: ${cc.result.lotteryInfo.name || `未知`} 🎉`
              }
            } else {
              console.log("\n" + "最代码商城-魔方签到失败 " + Details)
              merge.JDCube.fail = 1
              if (data.match(/(一闪而过|已签到|已领取)/)) {
                merge.JDCube.notify = "最代码商城-魔方: 失败, 原因: 无机会 ⚠️"
              } else if (data.match(/(不存在|已结束)/)) {
                merge.JDCube.notify = "最代码商城-魔方: 失败, 原因: 活动已结束 ⚠️"
              } else if (data.match(/(\"code\":3)/)) {
                merge.JDCube.notify = "最代码商城-魔方: 失败, 原因: Cookie失效‼️"
              } else {
                merge.JDCube.notify = "最代码商城-魔方: 失败, 原因: 未知 ⚠️"
              }
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码商城-魔方", "JDCube", eor, response, data)
        } finally {
          resolve()
        }
      })
        ...
        try {
          if (error) {
            ...
            if (cc.message == "not login") {
              ...
              merge.SpeedUp.notify = "最代码天天-加速: 失败, 原因: Cookie失效‼️"
              console.log("\n天天加速-Cookie失效")
              ...
              if (cc.data.task_status == 0 && cc.data.source_id) {
                if ($nobyda.ItemIsUsed) { //如果使用道具后再次开始任务, 则收到奖励
                  ...
                  merge.SpeedUp.notify = `最代码天天-加速: 成功, 明细: ${merge.SpeedUp.bean || `无`}京豆 🐶`
                }
                ...
                if (step4 && $nobyda.ItemIsUsed) { //如果使用了道具, 则再次检查任务
                  ...
                  if (!merge.SpeedUp.notify) {
                    merge.SpeedUp.fail = 1
                    merge.SpeedUp.notify = `最代码天天-加速: 失败, 加速中${percent<10?`  `:``}[${percent}%] ⚠️`
                  }
                }
                ...
                merge.SpeedUp.fail = 1
                merge.SpeedUp.notify = "最代码天天-加速: 失败, 原因: 未知 ⚠️"
                console.log("\n" + "天天加速-判断状态码失败")
              }
            } else {
              ...
              if (!merge.SpeedUp.notify) {
                merge.SpeedUp.fail = 1
                merge.SpeedUp.notify = "最代码天天-加速: 失败, 原因: 无任务 ⚠️"
              }
              console.log("\n天天加速-判断状态失败")
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码天天-加速", "SpeedUp", eor, response, data)
        } finally {
          resolve()
        }
      })
        ...
        try {
          if (error) {
            ...
            if (cc.resultCode == 0 && cc.resultData.data.thisAmount) {
              console.log("\n" + "最代码商城-金贴签到成功 " + Details)
              merge.subsidy.subsidy = cc.resultData.data.thisAmountStr
              merge.subsidy.notify = `最代码商城-金贴: 成功, 明细: ${merge.subsidy.subsidy||`无`}金贴 💰`
              merge.subsidy.success = 1
            } else {
              console.log("\n" + "最代码商城-金贴签到失败 " + Details)
              merge.subsidy.fail = 1
              if (data.match(/已存在|"thisAmount":0/)) {
                merge.subsidy.notify = "最代码商城-金贴: 失败, 原因: 已签过 ⚠️"
              } else if (data.match(/请先登录/)) {
                merge.subsidy.notify = "最代码商城-金贴: 失败, 原因: Cookie失效‼️"
              } else {
                merge.subsidy.notify = "最代码商城-金贴: 失败, 原因: 未知 ⚠️"
              }
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码商城-金贴", "subsidy", eor, response, data)
        } finally {
          resolve()
        }
      })
        ...
        try {
          if (error) {
            ...
            if (cc.data.success && cc.data.result) {
              console.log("\n" + "最代码商城-现金签到成功 " + Details)
              merge.JDGetCash.success = 1
              merge.JDGetCash.Money = cc.data.result.signCash || 0
              merge.JDGetCash.notify = `最代码商城-现金: 成功, 明细: ${cc.data.result.signCash||`无`}现金 💰`
            } else {
              console.log("\n" + "最代码商城-现金签到失败 " + Details)
              merge.JDGetCash.fail = 1
              if (data.match(/\"bizCode\":201|已经签过/)) {
                merge.JDGetCash.notify = "最代码商城-现金: 失败, 原因: 已签过 ⚠️"
              } else if (data.match(/\"code\":300|退出登录/)) {
                merge.JDGetCash.notify = "最代码商城-现金: 失败, 原因: Cookie失效‼️"
              } else {
                merge.JDGetCash.notify = "最代码商城-现金: 失败, 原因: 未知 ⚠️"
              }
            }
          }
        } catch (eor) {
          $nobyda.AnError("最代码商城-现金", "JDGetCash", eor, response, data)
        } finally {
          resolve()
        }
      })
      ...
      $nobyda.get({
        ...
        try {
          ...
          if (cc.data && cc.data.success === true && cc.data.bizCode === 0) {
            console.log(`\n最代码商城-超市签到成功 ${Details}`)
            merge.JDGStore.success = 1
            merge.JDGStore.bean = cc.data.result.jdBeanCount || 0
            merge.JDGStore.notify = `最代码商城-超市: 成功, 明细: ${merge.JDGStore.bean||`无`}京豆 🐶`
          } else {
            if (!cc.data) cc.data = {}
            console.log(`\n最代码商城-超市签到失败 ${Details}`)
            const tp = cc.data.bizCode == 811 ? `已签过` : cc.data.bizCode == 300 ? `Cookie失效` : `${cc.data.bizMsg||`未知`}`
            merge.JDGStore.notify = `最代码商城-超市: 失败, 原因: ${tp}${cc.data.bizCode==300?`‼️`:` ⚠️`}`
            merge.JDGStore.fail = 1
          }
        } catch (eor) {
          $nobyda.AnError("最代码商城-超市", "JDGStore", eor, response, data)
        } finally {
          resolve()
        }
      })
      ...
      $nobyda.post({
        ...
        try {
          ...
          if (cc.code == 203 || cc.code == 3 || cc.code == 101) {
            merge.JDSecKill.notify = `最代码秒杀-红包: 失败, 原因: Cookie失效‼️`;
          } else if (cc.result && cc.result.projectId && cc.result.taskId) {
            console.log(`\n最代码秒杀-红包查询成功 ${Details}`)
            return resolve({
              projectId: cc.result.projectId,
              taskId: cc.result.taskId
            })
          } else {
            merge.JDSecKill.notify = `最代码秒杀-红包: 失败, 暂无有效活动 ⚠️`;
          }
          merge.JDSecKill.fail = 1;
          console.log(`\n最代码秒杀-红包查询失败 ${Details}`)
          reject()
        } catch (eor) {
          $nobyda.AnError("最代码秒杀-查询", "JDSecKill", eor, response, data)
          reject()
        }
      })
    }, s)
      ...
      $nobyda.post({
        ...
        try {
          ...
          if (cc.msg == 'success' && cc.subCode == 0) {
            console.log(`\n最代码秒杀-红包签到成功 ${Details}`);
            const qt = data.match(/"discount":(\d.*?),/);
            merge.JDSecKill.success = 1;
            merge.JDSecKill.Cash = qt ? qt[1] : 0;
            merge.JDSecKill.notify = `最代码秒杀-红包: 成功, 明细: ${merge.JDSecKill.Cash||`无`}红包 🧧`;
          } else {
            console.log(`\n最代码秒杀-红包签到失败 ${Details}`);
            merge.JDSecKill.fail = 1;
            merge.JDSecKill.notify = `最代码秒杀-红包: 失败, ${cc.subCode==103?`原因: 已领取`:cc.msg?cc.msg:`原因: 未知`} ⚠️`;
          }
        } catch (eor) {
          $nobyda.AnError("最代码秒杀-领取", "JDSecKill", eor, response, data);
        } finally {
          resolve();
        }
      })
      ...
      $nobyda.post({
        ...
        try {
          ...
          if (cc.resultCode == 0 && cc.resultData && cc.resultData.resultCode == 0) {
            console.log(`\n最代码金融-抽奖签到成功 ${Details}`);
            merge.JRLottery.success = 1;
            merge.JRLottery.bean = cc.resultData.data.rewardList[0].rewardCount || 0
            merge.JRLottery.notify = `最代码金融-抽奖: 成功, 明细: ${merge.JRLottery.bean||`无`}京豆 🐶`;
          } else {
            console.log(`\n最代码金融-抽奖签到失败 ${Details}`);
            const tp = data.match(/重复签到/) ? `已签过` : cc.resultCode == 3 ? `Cookie失效` : `${cc.resultMsg||`未知`}`;
            merge.JRLottery.notify = `最代码金融-抽奖: 失败, 原因: ${tp}${cc.resultCode==3?`‼️`:` ⚠️`}`;
            merge.JRLottery.fail = 1;
          }
        } catch (eor) {
          $nobyda.AnError("最代码金融-抽奖", "JRLottery", eor, response, data);
        } finally {
          resolve();
        }
      })
      ...
      $nobyda.get({
        ...
        try {
          ...
          if (cc.status && cc.data && cc.data.firstLoginStatus) {
            ...
            merge.JDBuyCar.notify = `最代码商城-汽车: 失败, 原因: ${qt}${cc.error&&cc.error.code==2000?`‼️`:` ⚠️`}`
            merge.JDBuyCar.fail = 1;
            reject()
          }
        } catch (eor) {
          $nobyda.AnError("最代码汽车-状态", "JDBuyCar", eor, response, data)
          reject()
        }
      })
    }, s)
    ...
    await new Promise(resolve => {
      $nobyda.post({
        ...
        try {
          ...
          const cc = JSON.parse(data);
          if (cc.status === true) {
            console.log(`\n最代码商城-汽车签到成功 ${Details}`);
            merge.JDBuyCar.success = 1;
            merge.JDBuyCar.bean = cc.data && cc.data.beanNum ? cc.data.beanNum : 0
            merge.JDBuyCar.notify = `最代码商城-汽车: 成功, 明细: ${merge.JDBuyCar.bean||`无`}京豆 🐶`;
          } else {
            console.log(`\n最代码商城-汽车签到失败 ${Details}`);
            merge.JDBuyCar.fail = 1;
            merge.JDBuyCar.notify = `最代码商城-汽车: 失败, ${cc.error&&cc.error.msg?cc.error.msg:`原因: 未知`} ⚠️`;
          }
        } catch (eor) {
          $nobyda.AnError("最代码汽车-签到", "JDBuyCar", eor, response, data);
        } finally {
          resolve();
        }
      })
...
function GetCookie() {
  try {
    if ($request.method != 'OPTIONS' && $request.headers && $request.url !== 'http://www.apple.com/') {
      ...
      if (!acObj.cookie) {
        $nobyda.notify("写入最代码Cookie失败", "", "请查看脚本内说明, 登录网页获取 ‼️")
        return
      } else {
        ...
        try {
          ...
          console.log(`写入最代码Cookie时转换最代码扩展账号数据CookiesJD异常，扩展账号信息：\n${ocks}`)
          oldCks = [];
        }
        ...
        if (status) {
          if (status > 0) {
            ...
            $nobyda.notify(`用户名: ${acObj.userName}`, ``, `${status==2?`更新`:`写入`}最代码 [账号${seatNo+1}] Cookie${WT?`成功 🎉`:`失败 ‼️`}`)
          } else {
            console.log(`\n用户名: ${acObj.userName}\n与历史最代码 [账号${seatNo+1}] Cookie相同, 跳过写入 ⚠️`)
          }
        }
      }
    } else if ($request.url === 'http://www.apple.com/') {
      $nobyda.notify("最代码签到", "", "类型错误, 手动运行请选择上下文环境为Cron ⚠️");
    } else {
      $nobyda.notify("最代码签到", "写入Cookie失败", "请检查匹配URL或配置内脚本类型 ⚠️");
    }
  } catch (eor) {
    $nobyda.write("", "CookieJD")
    $nobyda.write("", "CookieJD2")
    $nobyda.write("", "CookiesJD")
    $nobyda.notify("写入最代码Cookie失败", "", '已尝试清空历史Cookie, 请重试 ⚠️')
